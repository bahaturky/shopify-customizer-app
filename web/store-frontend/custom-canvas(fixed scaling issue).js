...
//     if (brNew.top < 0 || brNew.left < 0 || brNew.top + brNew.height > canvas.height || brNew.left + brNew.width > canvas.width) {
//       // If so, stop the object at the canvas boundary.
//       obj.top = Math.max(0, obj.top);
//       obj.left = Math.max(0, obj.left);
//       obj.top = Math.min(canvas.height - brNew.height, obj.top);
//       obj.left = Math.min(canvas.width - brNew.width, obj.left);
//     }
//     return true;
//         // console.log(brNew.width, brNew.left, brNew.width+brNew.left, CANVAS_WIDTH, brNew.height, brNew.top, brNew.height+brNew.top,CANVAS_HEIGHT, brNew.left, minX, minY)
//         console.log(((brNew.width+brNew.left)>CANVAS_WIDTH) || ((brNew.height+brNew.top)>CANVAS_HEIGHT) || ((brNew.left<minX) || (brNew.top<minY)))
...
//           //   obj.top=top1;
//           // }
//           // if((brNew.width+brNew.left)>CANVAS_WIDTH || (brNew.height+brNew.top)>CANVAS_HEIGHT){
//           //   obj.set({
//           //       scaleX:scale1x,
//           //       scaleY:scale1y,
//           //       width:width1,
...
// // Check if the object is trying to go out of the canvas boundary.
// if (brNew.top < 0 || brNew.left < 0 || brNew.top + brNew.height > canvas.height || brNew.left + brNew.width > canvas.width) {
// // If so, stop the object at the canvas boundary.
// obj.top = Math.max(0, obj.top - (brNew.top < 0 ? brNew.top : 0));
// obj.left = Math.max(0, obj.left - (brNew.left < 0 ? brNew.left : 0));
// obj.top = Math.min(canvas.height - brNew.height, obj.top + (brNew.top + brNew.height > canvas.height ? brNew.top + brNew.height - canvas.height : 0));
// obj.left = Math.min(canvas.width - brNew.width, obj.left + (brNew.left + brNew.width > canvas.width ? brNew.left + brNew.width - canvas.width : 0));
// }
// });
...
//     // Check if the object is trying to go out of the canvas boundary.
//     if (brNew.top < minY || brNew.left < minX) {
//         // If the object exceeds the top or left boundary, stop it at the boundary.
//         obj.top = Math.max(0, obj.top + minY - brNew.top);
//         obj.left = Math.max(0, obj.left + minX - brNew.left);
//         return false;
//     }

//     if (brNew.top + brNew.height > CANVAS_HEIGHT || brNew.left + brNew.width > CANVAS_WIDTH) {
//         // If the object exceeds the bottom or right boundary, stop it at the boundary.
//         obj.top = Math.min(CANVAS_HEIGHT - brNew.height, obj.top + (CANVAS_HEIGHT - brNew.top - brNew.height));
//         obj.left = Math.min(CANVAS_WIDTH - brNew.width, obj.left + (CANVAS_WIDTH - brNew.left - brNew.width));
//         return false
//     }
//     return false
// });
...
$("#addTextContent").click(function(){
    ...
    console.log("before  text val => ", encodedValue);
    // text_val = text_val.replace(/\s+/g, " ");

    text_val = text_val.replace(/\n/g, "\n");
    console.log("after text val => ", typeof text_val, " | ", $("#textContent").val().replace(/\n/g, '\n'));
    if(text_val != ""){
        var font_size = 30;
        ...
        $.ajax({
          url: API_URL+"/textGenerate.php?text="+encodedValue+"&effect=normal&font_color=000000&font_size="+font_size+"&fontName=Abel&outline_color=00000000&outline_width=2",
          xhrFields: {
            responseType: 'blob'
          },
          beforeSend: function() {
...
function removeTxtOutline(){
    ...
    $.ajax({
      url: API_URL+"/textGenerate.php?text="+encodeURIComponent(selectedObject.text)+"&effect="+selectedObject.text_effect+"&font_color="+selectedObject.text_color.replace("#","")+"&font_size="+selectedObject.text_font_size+"&fontName="+selectedObject.text_font_family+"&outline_color=00000000&outline_width="+1,
      xhrFields: {
        responseType: 'blob'
      },
      beforeSend: function() {
