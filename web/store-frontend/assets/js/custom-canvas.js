...
canvas1.setAttribute('id','tempCanvas');
const ctx = canvas1.getContext('2d');


const os = document.createElement('canvas');
os.setAttribute('id','osCanvas');
const octx = os.getContext('2d');

octx.font = '64px impact';
        ...
        if(product_map.length > 0){
          for(var a=0;a<product_map.length;a++){
            ...
            if(a == 0){
              ...
              os.width = CANVAS_WIDTH;
              os.height = CANVAS_HEIGHT;
              
              angleSteps = 180 / CANVAS_WIDTH;
            }
  ...
  event.target.lockScalingFlip = false;
    if (event.target.object_type == 'text') {
      event.target.lockScalingFlip = true;
        // let newWidth = (event.target.width * event.target.scaleX);
        // let newHeight = (event.target.height * event.target.scaleY);
        // console.log("New font size => ",newHeight)
        ...
        //   tempCtx.font = `${fontSize}px ${event.target.text_font_family}`;

        //   // Measure the width and height of the text
        //   const textWidth = tempCtx.measureText(event.target.text).width;
        //   const textHeight = fontSize;

        //   // Create a temporary canvas with auto dimensions to draw the text
        //   const canvasWidth = textWidth; // Add padding to the text width
        //   const canvasHeight = textHeight; // Add padding to the text height
        ...
        //   ctx.font = `${fontSize}px ${event.target.text_font_family}`;
        //   ctx.fillStyle = 'red'; // Set text color
        //   ctx.textAlign = 'center'; // Center the text

        //   // Calculate the y-coordinate to vertically center the text
        //   const textY = (canvasHeight - fontSize) / 2 + fontSize * 0.8;

        //   // Draw the text on the temporary canvas
        //   ctx.fillText(event.target.text, canvasWidth / 2, textY);
        ...
        //   event.target.setSrc(autoCanvas.toDataURL());
        ...
        // let newFontVal = calFontSize(null, parseFloat(event.target.text_font_size));
            // if(parseFloat(event.target.scaleToWidth) !== parseFloat(newWidth) && checkZoomModify == false){
              ...
              // var scaleFactor = Math.min(newWidth / event.target.width, newHeight / event.target.height);

              // event.target.set({
              //           scaleToWidth:parseFloat(newWidth),
              //           scaleToHeight:parseFloat(newHeight),
              //           width: parseFloat(newWidth),
              //           height: parseFloat(newHeight), 
              //           scaleX: scaleFactor,
              //           scaleY: scaleFactor,
              //           "text_font_size": newFontVal
              //   })
...
// code for text shape effect startOver

function renderNormalText(selectedObject, effect){
  // Create a temporary canvas to measure the text dimensions
      const tempCanvas = document.createElement('canvas');
      const tempCtx = tempCanvas.getContext('2d');
      ...
      // Style the text (for measuring purposes)
      const fontSize = 120;
      tempCtx.font = `${fontSize}px ${selectedObject.text_font_family}`;

      // Measure the width and height of the text
      const textWidth = tempCtx.measureText(selectedObject.text).width;
      const textHeight = fontSize;
      ...
      const ctx = autoCanvas.getContext('2d');
      // Style the text (for drawing purposes)
      ctx.font = `${fontSize}px ${selectedObject.text_font_family}`;
      ctx.fillStyle = selectedObject.text_color_name; // Set text color
      ctx.textAlign = 'center'; // Center the text
      ...
      // Draw the text on the temporary canvas
      ctx.fillText(selectedObject.text, canvasWidth / 2, textY);
      ...
      const img = new Image();
      img.src = autoCanvas.toDataURL();
      selectedObject.setSrc(autoCanvas.toDataURL());
...
function renderBridgeText(selectedObject, effect) {
    ...
    const os = document.createElement('canvas');
    const octx = os.getContext('2d');
    os.width = screen.width * 2;
    os.height = screen.height * 2;
    octx.font = `${fontSize}px ${selectedObject.text_font_family}`;
    octx.textBaseline = 'top';
    // octx.textAlign = 'center';

    const width = octx.measureText(selectedObject.text).width;
    ...
    canvas1.height = height;
        ...
        octx.fillText(selectedObject.text, 0, 0);
...
// End code for text shape effect
...
$("#addTextContent").click(function(){
    ...
    if(text_val != ""){
        ...
        console.log("Font size => ", newFontVal);
        ...
        // Create a temporary canvas to measure the text dimensions
          ...
          // Style the text (for measuring purposes)
          const fontSize = newFontVal;
          tempCtx.font = `${fontSize}px ${font_name}`;

          // Measure the width and height of the text
          const textWidth = tempCtx.measureText(text_val).width;
          const textHeight = fontSize;
          ...
          // Draw the text on the temporary canvas
          ctx.fillText(text_val, canvasWidth / 2, textY);

          // Create an image object from the temporary canvas
          const img = new Image();
          img.src = autoCanvas.toDataURL();
          // console.log(autoCanvas.toDataURL())
...
function changeTextEffect(effect){
    ...
    switch (effect) {
            case 'normal':
                renderNormalText(selectedObject, effect);
                break;
            // Add cases for other effects
            case "curve":
                renderCurveText(encodeURIComponent(selectedObject.text));
                break;

            case "arch":
                renderArchText(encodeURIComponent(selectedObject.text))
                break;

            case "bridge":
                renderBridgeText(selectedObject, effect)
                break;

            case "valley":
                renderValleyText(encodeURIComponent(selectedObject.text))
                break;

            case "pinch":
                renderPinchText(encodeURIComponent(selectedObject.text))
                break;

            case "bulge":
                renderBulgeText(encodeURIComponent(selectedObject.text))
                break;

            case "perspective":
                renderPerspectiveText(encodeURIComponent(selectedObject.text))
                break;

            case "pointed":
                renderPointedText(encodeURIComponent(selectedObject.text))
                break;

            case "downward":
                renderDownwardText(encodeURIComponent(selectedObject.text))
                break;

            case "upward":
                renderUpwardText(encodeURIComponent(selectedObject.text))
                break;

            case "cone":
                renderConeText(encodeURIComponent(selectedObject.text))
                break;
